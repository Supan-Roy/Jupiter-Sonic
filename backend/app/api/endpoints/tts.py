...
class TTSRequest(BaseModel):
    ...
    voice_print_path: str = Field(
        ..., description="Local file path of the cloned voice print"
    )
    output_filename: str | None = Field(
        None, description="Optional custom filename (must end in .wav)"
    )


@router.post("/synthesize", response_model=TTSResult)
async def synthesize_speech(
    payload: TTSRequest, tts_module: TTSModuleInterface = Depends(get_tts_module)
):

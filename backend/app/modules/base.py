...
class WordTimestamp(BaseModel):
    ...
    probability: float | None = Field(None, description="Confidence score")
...
class ASRResult(BaseModel):
    ...
    segments: list[ASRSegment] = Field(
        default_factory=list, description="Text segments with timestamps"
    )
...
class EnhancementResult(BaseModel):
    enhanced_audio_path: str = Field(
        ..., description="Path to noise-reduced clean audio"
    )
    snr_db: float | None = Field(
        None, description="Signal-to-noise ratio estimate in dB"
    )
...
class DubbingResult(BaseModel):
    dubbed_video_path: str = Field(
        ..., description="Path to output video with aligned dubbed audio"
    )
    output_audio_path: str = Field(
        ..., description="Path to isolated dubbed audio track"
    )
    success: bool = True
...
class TTSModuleInterface(ABC):
    ...
    async def synthesize(
        self, text: str, voice_print_path: str, output_path: str
    ) -> TTSResult:

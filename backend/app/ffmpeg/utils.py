...
async def run_ffmpeg_cmd(args: list[str]) -> bool:
    ...
    try:
        ...
        if process.returncode == 0:
            ...
            logger.error(f"FFmpeg failed with exit code {process.returncode}. error: {stderr.decode()}")
            return False
    except FileNotFoundError:
        logger.error(f"FFmpeg executable not found at path: {settings.FFMPEG_PATH}. Please install FFmpeg.")
        return False
    except Exception as e:
        logger.error(f"Failed to execute FFmpeg command: {e}")
        return False

async def convert_to_speech_wav(input_path: str, output_path: str, sample_rate: int = 16000) -> bool:
...
async def extract_audio(video_path: str, output_audio_path: str) -> bool:
    ...
    args = [
        ...
        "-acodec", "copy",     # Copy audio codec (no re-encoding, extremely fast)
        output_audio_path
    ]
...
async def merge_audio_video(video_path: str, audio_path: str, output_video_path: str) -> bool:
